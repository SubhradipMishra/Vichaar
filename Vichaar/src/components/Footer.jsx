...
const footerLinks = {
  Explore: [
    ...
    { label: 'Templates', href: '#', emoji: '✨' },
    { label: 'Community', href: '#community', emoji: '💬' },
    { label: 'Use Cases', href: '#', emoji: '🎯' },
    { label: 'Changelog', href: '#', emoji: '🚀' },
    { label: 'Open Source', href: '#', emoji: '🌍' },
  ],
  Product: [
    { label: 'Features', href: '#features', emoji: '⚡' },
    { label: 'Pricing', href: '#pricing', emoji: '💎' },
    { label: 'For Teams', href: '#', emoji: '👥' },
    { label: 'AI Writing API', href: '#', emoji: '🔌' },
    { label: 'Integrations', href: '#', emoji: '🔗' },
    { label: 'Roadmap', href: '#', emoji: '🗺️' },
  ],
  Resources: [
    { label: 'Documentation', href: '#', emoji: '📚' },
    { label: 'Help Center', href: '#', emoji: '🆘' },
    { label: 'Privacy Policy', href: '#', emoji: '🔒' },
    { label: 'Terms of Use', href: '#', emoji: '📋' },
    { label: 'Security', href: '#', emoji: '🛡️' },
    { label: 'Status Page', href: '#', emoji: '✅' },
  ],
  'Contact Us': [
    { label: 'hello@vichaar.ai', href: 'mailto:hello@vichaar.ai', emoji: '📧' },
...
const socials = [
  {
    label: 'X', color: '#1DA1F2', bg: '#e8f5fe',
  ...
  {
    label: 'GitHub', color: '#333', bg: '#f3f4f6',
  ...
  useEffect(() => {
    ...
    if (!erasing) {
      if (shown.length < word.length) {
        const t = setTimeout(() => setShown(word.slice(0, shown.length + 1)), 80)
        return () => clearTimeout(t)
      }
      if (idx < taglines.length - 1) {
        const t = setTimeout(() => setErasing(true), 700)
        return () => clearTimeout(t)
      }
    } else {
      if (shown.length > 0) {
...
function FooterLink({ link }) {
  ...
  const inner = (
    <>
      <motion.span animate={{ x: hov ? 0 : -4, opacity: hov ? 1 : 0 }} transition={{ duration: 0.2 }}
        className="text-xs">{link.emoji}</motion.span>
      <span>{link.label}</span>
      <motion.span animate={{ x: hov ? 0 : -6, opacity: hov ? 1 : 0 }} transition={{ duration: 0.2 }}
        className="text-xs ml-auto">→</motion.span>
    </>
  )
    ...
    <footer className="relative overflow-hidden" style={{ background: '#0a0917' }}>
      ...
      <div className="w-full overflow-hidden leading-none" style={{ marginBottom: -2 }}>
        <svg viewBox="0 0 1440 80" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ display: 'block', width: '100%' }}>
          <path d="M0 80 C240 20 480 60 720 40 C960 20 1200 60 1440 30 L1440 0 L0 0 Z"
            fill="#f8f7ff" />
        </svg>
      </div>
      ...
      <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{ zIndex: 0 }}>
        <motion.div animate={{ scale: [1, 1.15, 1], opacity: [0.12, 0.18, 0.12] }}
        ...
        <motion.div animate={{ scale: [1, 1.1, 1], opacity: [0.08, 0.14, 0.08] }}
      ...
      <div className="relative z-10 max-w-7xl mx-auto px-6 lg:px-12">
        ...
        {/* ── Main grid ── */}
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-6 gap-12 py-14 border-b"
          style={{ borderColor: 'rgba(255,255,255,0.07)' }}>
          ...
          <div className="lg:col-span-2">
            ...
            <p className="text-sm text-gray-400 leading-relaxed mb-6 max-w-xs">
              The AI-powered blogging platform built for writers who think deeply and publish boldly. ✍️
            </p>

            {/* Social icons with fancy hover */}
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 py-6">
          <p className="text-xs text-gray-600">
            © 2025 Vichaar · Crafted with ❤️ for writers everywhere
          </p>
